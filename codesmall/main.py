...
if args.debug:
    ...
    if world_size == 8:
        train_dataset = D('../data/train_full_%d.npy'%args.local_rank, training=True)
    elif world_size == 4:
        train_dataset = ConcatDataset([
            D('../data/train_full_%d.npy'%(args.local_rank*2), training=True),
            D('../data/train_full_%d.npy'%(args.local_rank*2+1), training=True),
        ])
    elif world_size == 1:
        train_dataset = ConcatDataset([
            D('../data/train_full_%d.npy'%(i), training=True) for i in range(8)
        ])
